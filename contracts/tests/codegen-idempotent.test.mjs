...
test("generated/ 下存在 39 个代码文件（13 schema × 3 端）", () => {
  const files = listGeneratedFiles();
  assert.equal(files.length, 39, `expected 39 generated files, got ${files.length}`);
  // 每端各 13 个
  ...
  assert.equal(swift, 13, `swift 端文件数应为 13，实际 ${swift}`);
  assert.equal(kotlin, 13, `kotlin 端文件数应为 13，实际 ${kotlin}`);
  assert.equal(arkts, 13, `arkts 端文件数应为 13，实际 ${arkts}`);
});
...
test("codegen 幂等性：跑前后所有文件 byte-for-byte 一致", () => {
  const files = listGeneratedFiles();
  assert.ok(files.length === 39, "generated 文件数应为 39");
