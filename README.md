...
Current Reader-UI Contract head is 3.1.0: Route 260, UiEvent 300, Motion 95, CoreCommand 72, CoreEvent 95, HostRequest 58, HostResult 58, schema-scanned fixtures 1333, runtime actions 67, ownership 67 implemented / 226 planned / 7 platform-ephemeral. R14.1 defines strict lossless payload contracts for all 67 actions (190 payload fixtures, 77 result mappings across 39 effect types, and 162 result fixtures). R16D currently publishes a 260-route / 190-variant / 615-component ScreenGraph with 97 canonical action bindings: 41 bind to the executable runtime payload contract and 56 remain planned/fail-closed; it also carries 19 non-executable state-event evidence records and 6 explicit action gaps. Component nodes publish state authority and `contract-tree` / `host-composite` composition semantics so native renderers do not recursively duplicate Host-owned Reader surfaces. The current 3.1 candidate release manifest is generated from the current repository bytes; run its check command instead of copying historical file/test counts from this paragraph. The three Host consumer locks remain at 2.5.1 and cover the same 35 runtime events: directory, `book.open`, TTS, and auto-page form the 7-event Pilot set; page, import, source-switch, replace, RSS, and Sync remain the 28-event default-Shadow set. The other 32 runtime actions remain staging-only, and no workflow is Authoritative yet. Reader-UI 3.1.0 therefore does not claim release consumption until the same bytes are committed/tagged, a verified 3.1 artifact is published, and each Host lock is updated from that artifact.
