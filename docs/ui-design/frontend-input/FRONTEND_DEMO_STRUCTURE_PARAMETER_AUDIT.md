...
当前 demo 已经能承载主要交互和页面结构；` 2` 副本已从正式项目中删除，结构唯一性问题已收敛。剩余主要风险是 demo renderer / CSS 仍然过大，部分参数和文档口径仍需继续拆分与同步。

| 维度（Dimension） | 结论（Conclusion） | 严重度（Severity） |
|---|---|---|
| 项目结构（Project Structure） | ` 2` 副本已删除，正式源唯一；但 demo 仍集中在大体量 renderer / CSS。 | 中 |
...
| 文档同步（Documentation Sync） | demo 已移除或改造的内容，在部分 planning docs 中仍可能保留旧口径，例如独立缓存管理页、同步备份记录模型、协议入口。 | 中高 |
...
|---|---:|---|
| `frontend-demo-draft/render.js` | 5004 行 | 路由、页面模板、状态、事件绑定、硬编码内容混在一个文件。 |
| `frontend-demo-draft/styles.css` | 9547 行 | 视觉样式过大，组件分区不够清晰。 |
...
3. **拆分 demo renderer（Split Renderer）**：按 `main-tabs / library / reader / settings / source-management` 拆 `render.js`。
4. **拆分 CSS（Split Styles）**：按 `tokens / shell / components / page` 分层，逐步减少散落 px。
...
- 正式源已收敛为无 ` 2` 路径；后续清理重点转向过期规划、route 命名、代码和样式拆分。
