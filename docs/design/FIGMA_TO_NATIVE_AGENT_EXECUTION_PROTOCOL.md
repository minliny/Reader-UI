...
| Independent native A2 cleanup must precede promotion. | `FIGMA_VISUAL_ADMISSION_DEPENDENCIES.json.nativeA2ConsumerClosures` indexes an immutable receipt outside the B3 handoff directory. `promote-family.mjs` verifies the exact B3 packet/hash, A2 delta/hash, complete record set, HarmonyOS cleanup commit/tree/diff, commit ancestry, and the known Screen Graph check before any transaction write. A historical-bootstrap receipt can validate an existing promotion but can never authorize a new or replayed promotion. |
| Declared admission dependencies are executable constraints. | `FIGMA_VISUAL_ADMISSION_DEPENDENCIES.json.dependencies[]` is checked both before promotion and by `--check`; a page family cannot become or remain implementation-ready while a required record has the wrong local or Harmony admission status. |
...
| B3 evidence must be committed and clean. | The packet itself must be tracked and clean, `admission.exactLocalCommit` must equal `localSource.implementationCommit`, and that implementation commit must be an ancestor of the candidate HEAD. |
