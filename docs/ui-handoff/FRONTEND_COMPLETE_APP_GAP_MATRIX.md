...
| --- | --- | --- |
| Demo route render | Browser smoke verified `200/200` `captureRoute` pages open with non-empty active stage. | Demo is useful as canonical visual and interaction sample, and now covers every `route.schema.json` id. |
| Motion coverage | `node frontend-demo/verify/motion/verify-motion-coverage.mjs` passes `30/30`; route render coverage `200/200`, canonical MotionSpec fixture registry `84/84`, runtime/selector alias resolution `83/83`, selector data coverage `151/184`. | Demo motion registry and contract fixture registry are strong, but not a full native implementation. |
| Contract tests | `node --test contracts/tests/*.test.mjs` passes `171/171`. | Schema, fixtures, generated type consistency, slice fixtures, P0 matrix, full MotionSpec guard, token group, Core/Host boundary checks, and demo strict/exception consistency are healthy. |
...
| Handoff readiness | `verify-ui-handoff-readiness.mjs` passes `8/8`; contract-only `Package.swift` is explicitly allowed and other production entries still fail. | Handoff policy matches the Swift contract package role. |
| Demo contract consistency | `verify-demo-contract-consistency.mjs` reports `found=515 unknown=111 approved=111 unapproved=0`. | Route/token unknown are hard-gated at 0; motion unknown is allowed only through `demo-contract-exceptions.json`. This is a strict/exception starting point, not 0 drift. |
| Route source status | `route.schema.json` has `200` route ids; `route-contract.js` and `renderRoute()` both cover `200/200` with route unknown `0`. | Contract is the canonical superset; the demo now covers every contract route id, but does not prove native route delivery. |
| Motion spec completeness | `motion.schema.json` has `84` MotionId values; `motion.fixtures.json` has `84` specs. P0 `40/40` and full schema `84/84` MotionId fixtures are present, token refs are valid, and guardRules are tested. | Contract-side MotionSpec registry is complete; native MotionAdapter/device proof remains platform work. |
...
| L1: contract input ready | Schema, fixtures, codegen, and demo proof can guide platform work. | Achieved for Reader UI handoff. |
| L2: hard contract gate ready | Demo, schema, generated files, token, and motion ids have zero drift. | Achieved for route/render handoff gates; motion exception burn-down remains open. |
...
| --- | --- | --- | --- | --- | --- |
| P0-01 | Canonical route source needs policy lock | Reader UI | Schema `200` ids; demo route contract and renderer now cover `200/200`, route unknown `0`. | Platforms can still confuse demo coverage with full native route delivery. | Keep contract as canonical source; every native route claim must cite schema id, priority, owner, and evidence. |
...
| P0-04 | Full MotionSpec registry platform consumption | Reader UI + Platforms | Reader UI now has `84/84` canonical MotionSpec fixtures; codegen emits Swift/Kotlin/ArkTS `MotionSpecRegistry`; tests enforce schema/fixture 1:1, token refs, guardRules, and reduced/direct/interrupt semantics. | Platforms can still drift if they copy enum names but bypass generated registry or ignore reduced-motion/guardRules. | Each platform MotionAdapter consumes generated registry, maps reduced-motion to native animation APIs, and provides compile/test/device evidence. |
...
| P0-06 | Handoff readiness policy conflicts with contract package | Reader UI | Closed: verifier allows only the contract Swift package shape for `Package.swift`; readiness passes `8/8`. | Remaining risk is future accidental production entries, which still fail the gate. | Keep the contract-only package predicate narrow: `ReaderUIContract`, `generated/swift`, and no executable target. |
...
| P1-04 | Selector coverage is not complete | Reader UI | Motion verifier reports `151/184` `data-*` directly mapped. | Some demo interactions still rely on fallback or implicit mapping. | Either map all interaction selectors or explicitly classify non-interactive metadata/debug selectors. |
...
| ID | Gap | Owner | Current evidence | Impact | Acceptance |
| --- | --- | --- | --- | --- | --- |
| P2-01 | Demo renderer maintainability | Reader UI | `frontend-demo/render-runtime.js` is over 10k lines and owns 200 route cases. | Changes are risky and hard to review. | Split renderer by shell or feature without changing route/motion/state output. |
| P2-02 | Full 200-route native migration strategy | Platforms | Slice matrix warns against all-at-once route migration. | Teams may attempt broad shallow parity and miss behavior. | Maintain route priority tiers: P0 slices first, P1 business flows next, P2 long tail last. |
