...
- Demo 已接入 `motion.interrupt.cancel/redirect/completeThenReplace` 第一版 state adapter，覆盖 route push/replace/back、Tab 切换、viewport 变化、loading 完成、宽屏 dock 拖动开始和 pointer cancel，输出 `data-motion-interrupt-*` 并清理 pressed/dragging/dropdown 临时态；overlay/focus 第一版状态字段已接入，连续 overlay 打断、连续下拉 A->B、异步结果防覆盖和录屏证据仍需补齐。
