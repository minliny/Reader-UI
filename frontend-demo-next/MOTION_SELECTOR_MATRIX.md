...
> 继承说明：本文保留 optimized/canonical 基线的 selector 审计快照与原证据索引；当前 candidate 的可执行覆盖结果以 `verify/motion/motion-coverage-report.json` 为准。证据路径中的 `frontend-demo/` 表示授权提升后的 canonical 目标，不是 candidate 验证器的读写路径。
