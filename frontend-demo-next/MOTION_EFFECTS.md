...
- to: `controlLayerVisible`
- interrupt: `hideAgain`, `routeChange`, `orientationPrepare`, `sessionStart`
- finalState: `controlLayerVisibleAndImmersiveHotZonesCleared`
- reducedMotion: 控制层即时显示，不做 slide/fade；正文排版保持稳定。
...
1. D5 动效闭环已完成：15 类动效（首次打开、Tab press/select/switch、dropdown 展开/关闭/A→B 切换、封面进入阅读、控制层显隐、小横条拖动、平板 dock 拖动与回弹、Reader 模块切换、TTS/自动翻页胶囊、overlay 连续打断、异步结果过期、旋转/resize、reduced motion、焦点恢复）全部接入 contract 状态机、token 化 CSS、demo adapter 和 reduced-motion 降级。`reader.control.show` 已补全精确状态机。Motion coverage 30/30 通过。
2. 已完成第一版：`motion-tokens.css`、裸写时长替换、reduced-motion CSS/查询开关、`MOTION_SELECTOR_MATRIX.md`、基础 `data-motion-id` / pressed state 和 `data-motion-component-*` normalized adapter 接入。
