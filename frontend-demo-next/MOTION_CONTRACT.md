...
| `reader.control.show` | `immersive-reading` -> `reader` | 阅读控制层覆盖在同一个阅读正文层之上；正文层不能重排、变暗或改变透明度。顶部阅读栏从上方 `-8px` 淡入，底部控制面板从下方 `12px` 淡入，模块导航延迟不超过 `20ms`。 |
...
- D5 动效闭环已完成：15 类动效（首次打开、Tab press/select/switch、dropdown 展开/关闭/A→B 切换、封面进入阅读、控制层显隐、小横条拖动、平板 dock 拖动与回弹、Reader 模块切换、TTS/自动翻页胶囊、overlay 连续打断、异步结果过期、旋转/resize、reduced motion、焦点恢复）全部接入 contract 状态机、token 化 CSS、demo adapter 和 reduced-motion 降级。Motion coverage 30/30 通过。
- `reader.control.show` 已补全精确状态机（from `immersiveReading` -> to `controlLayerVisible`，interrupt `hideAgain/routeChange/orientationPrepare/sessionStart`），与 `reader.control.hide` 对称。
