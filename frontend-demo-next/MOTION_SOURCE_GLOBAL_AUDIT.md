...
> 继承说明：本文是 2026-06-26 optimized/canonical 基线的历史快照，其中 route 数量、行号与覆盖统计不代表当前 candidate。当前事实以 `verify/motion/motion-coverage-report.json` 和 candidate README 为准；可执行命令按当前 demo 目录使用相对路径。
...
- 本地加载 `http://127.0.0.1:5177/<demo-directory>/?motionReduced=1&captureRoute=<route>`。
...
```bash
node --check motion-controller.js
```

```bash
node verify/motion/verify-motion-coverage.mjs
```
