...
脚本会检查当前 demo 目录的 route 覆盖、Motion selector 覆盖、`motion-controller.js` 加载顺序和 runtime 接入点，并输出：

- `verify/motion/motion-coverage-report.json`

同时会检查：

- `verify/motion/evidence/manifest.json`
- `verify/motion/evidence/*`
