...
// 用法：node frontend-demo-next/verify/verify-p0-chain-matrix.mjs
...
const READER_UI_ROOT = resolve(__dirname, "..", "..");
const WORKSPACE_ROOT = resolve(READER_UI_ROOT, "..");
const REPO_PATHS = {
  Contract: READER_UI_ROOT,
  iOS: join(WORKSPACE_ROOT, "Reader-for-iOS"),
  Android: join(WORKSPACE_ROOT, "Reader-for-Android"),
  HarmonyOS: join(WORKSPACE_ROOT, "Reader-for-HarmonyOS"),
};
...
function verifyIOS(chain) {
  ...
  const appShell = readText(join(appDir, "AppShellView.swift")) || "";
  ...
  const bookshelfMotion = readText(join(iosDir, "iOS", "Features", "Bookshelf", "BookshelfView.swift")) || "";
  const readerMotion = readText(join(iosDir, "iOS", "Features", "Reader", "ReaderView.swift")) || "";
  const settingsMotion = [
    readText(join(iosDir, "iOS", "Features", "Settings", "SettingsTabView.swift")) || "",
    readText(join(iosDir, "iOS", "Features", "Settings", "SettingsDemoShellView.swift")) || "",
  ].join("\n");
  ...
  // E. motion：通用 Adapter 可解析 MotionRequest，且生产调用点按链路提供上下文。
  // Adapter 本身不应依赖 route 名字；source-switch/book-detail 由 AppShell 的
  // route → containerRole 映射接入，bookshelf/reader/settings 在各自 View 调用。
  const hasResolverAdapter = testRegex(motionAdapter, /ReaderMotionResolver\.resolve\(request\)/)
    && testRegex(motionAdapter, /static func animation\s*\(\s*for request:/);
  const hasAdapterCall = (content) => testRegex(content, /ReaderMotionAdapter\.animation\s*\(/)
    && testRegex(content, /MotionRequest\s*\(/);
  const motionWiring = {
    bookshelf: hasAdapterCall(bookshelfMotion),
    reader: hasAdapterCall(readerMotion),
    "source-switch": hasAdapterCall(appShell)
      && testRegex(appShell, /case \.sourceSwitch:\s*return \.flowShell/s),
    "book-detail": hasAdapterCall(appShell)
      && testRegex(appShell, /case \.bookDetail, \.bookDetailToc:\s*return \.libraryShell/s),
    settings: hasAdapterCall(settingsMotion)
      && testRegex(settingsMotion, /containerRole:\s*\.settingsShell/),
  };
  const E = hasResolverAdapter && motionWiring[chain];
...
function generateMarkdown(matrix) {
  ...
  lines.push("");
  lines.push("> 由 `frontend-demo-next/verify/verify-p0-chain-matrix.mjs` 自动生成");
  ...
  lines.push("- E. motion：ReaderMotionAdapter 可解析 MotionRequest，且生产 View/AppShell 有该链路上下文接线");
