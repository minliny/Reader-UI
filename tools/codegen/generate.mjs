...
// Route reconstruction quarantine is a source-side retirement ledger for
// historical native route consumption. It deliberately preserves published
// RouteId values, but tells consumers that the old shell/body mapping must not
// be generated until a fresh Figma → Reader-UI conversion is completed.
//
// This is NOT a HarmonyOS-local policy or a new visual authority. The one
// fixture below is generated for every platform and is also consumed when the
// HarmonyOS RouteTable/ViewStateTable are produced.
function normalizeRouteReconstructionQuarantine(document, routeSchema) {
  if (document === null || Array.isArray(document) || typeof document !== "object") {
    throw new Error("fixtures/route-reconstruction-quarantine.fixtures.json must contain one object");
  }
  if (document.schemaVersion !== 1) {
    throw new Error("route reconstruction quarantine schemaVersion must be 1");
  }
  if (document.status !== "active" && document.status !== "released") {
    throw new Error("route reconstruction quarantine status must be 'active' or 'released'");
  }
  if (!Array.isArray(document.entries) || document.entries.length === 0) {
    throw new Error("route reconstruction quarantine entries must be a non-empty array");
  }

  const knownRouteIds = new Set(extractEnum(routeSchema, "id") || []);
  const seenRecordIds = new Set();
  const seenRouteIds = new Set();
  const entries = document.entries.map((entry, index) => {
    const label = `route reconstruction quarantine entry ${index + 1}`;
    if (entry === null || Array.isArray(entry) || typeof entry !== "object") {
      throw new Error(`${label} must be an object`);
    }
    if (typeof entry.recordId !== "string" || entry.recordId.trim().length === 0) {
      throw new Error(`${label}.recordId must be a non-empty string`);
    }
    if (seenRecordIds.has(entry.recordId)) {
      throw new Error(`${label}.recordId '${entry.recordId}' is duplicated`);
    }
    seenRecordIds.add(entry.recordId);
    if (!Array.isArray(entry.routeIds) || entry.routeIds.length === 0) {
      throw new Error(`${label}.routeIds must be a non-empty array`);
    }
    ...
    const routeIds = entry.routeIds.map((routeId) => {
      ...
      if (seenRouteIds.has(routeId)) {
        throw new Error(`${label}.routeIds repeats '${routeId}' across the source quarantine`);
      }
...
function genSwiftRouteReconstructionQuarantine(quarantine) {
  const active = quarantine.status === "active";
  const recordIds = active ? quarantine.recordIds : [];
  const routeIds = active ? quarantine.routeIds : [];
...
function genKotlinRouteReconstructionQuarantine(quarantine) {
  const active = quarantine.status === "active";
  const recordIds = active ? quarantine.recordIds : [];
  const routeIds = active ? quarantine.routeIds : [];
  return `// AUTO-GENERATED by tools/codegen/generate.mjs. DO NOT EDIT.
...
function genArkTsRouteReconstructionQuarantine(quarantine) {
  const active = quarantine.status === "active";
  const recordIds = active ? quarantine.recordIds : [];
  const routeIds = active ? quarantine.routeIds : [];
  return `// AUTO-GENERATED by tools/codegen/generate.mjs. DO NOT EDIT.
// Source: contracts/route-reconstruction-quarantine.schema.json + fixtures/route-reconstruction-quarantine.fixtures.json
export class RouteReconstructionQuarantine {
  ...
  static contains(routeId: string): boolean {
    return RouteReconstructionQuarantine.STATUS === "active" &&
      RouteReconstructionQuarantine.ROUTE_IDS.indexOf(routeId) >= 0;
  }

  static blocksPromotion(recordId: string): boolean {
    return RouteReconstructionQuarantine.STATUS === "active" &&
      RouteReconstructionQuarantine.RECORD_IDS.indexOf(recordId) >= 0;
  }
}
`;
}
...
function generate() {
  ...
  const routeReconstructionQuarantine = normalizeRouteReconstructionQuarantine(
    loadFixtures("route-reconstruction-quarantine"),
    routeSchema,
  );
  ...
  writeFile(join(GENERATED_DIR, "swift", "RouteReconstructionQuarantine.swift"), genSwiftRouteReconstructionQuarantine(routeReconstructionQuarantine));
  ...
  writeFile(join(GENERATED_DIR, "kotlin", "RouteReconstructionQuarantine.kt"), genKotlinRouteReconstructionQuarantine(routeReconstructionQuarantine));
  ...
  writeFile(join(GENERATED_DIR, "arkts", "RouteReconstructionQuarantine.ets"), genArkTsRouteReconstructionQuarantine(routeReconstructionQuarantine));
  ...
  console.log(`[codegen] Route reconstruction quarantine: status=${routeReconstructionQuarantine.status}, routes=${routeReconstructionQuarantine.routeIds.length}, records=${routeReconstructionQuarantine.recordIds.length}`);
}
