...
export function recoverRepinJournalWhileLocked(opts = {}) {
  ...
  if (newHashes && specNew && depsNew) {
    ...
    // At least one live file matches NEITHER its original preimage NOR the new
    // post-image: an UNKNOWN live hash (a concurrent external mutation, or drift
    // from a different repin). Known-hash recovery is authorized to auto-complete
    // or auto-rollback; an unknown hash is NOT - restoring the backup here would
    // OVERWRITE those foreign bytes (data loss). Fail closed for manual
    // resolution: do NOT mutate any live file, and let the throw skip cleanup()
    // below so the journal + temps + backups are preserved for inspection. The
    // caller (recoverRepinJournal) releases the writer lock in its finally. Phase
    // markers are DIAGNOSTIC ONLY here - they never authorize overwriting unknown
    // data: a completed-then-third-party-mutated transaction still has unknown
    // live hashes regardless of an 'applied' marker.
    const classify = (live, orig, neu) =>
      live === null ? "absent" :
      neu && live === neu ? "new" :
      orig !== null && live === orig ? "original" : "UNKNOWN";
    const specClass = classify(liveSpecHash, originalSpecHash, newHashes ? newHashes.spec : null);
    const depsClass = classify(liveDepsHash, originalDepsHash, newHashes ? newHashes.deps : null);
    const phaseList = document.phases.map((p) => p.phase).join(",");
    throw new Error(
      `[runtime-payload-source] manual resolution required: a live file has an ` +
      `unknown hash (spec=${specClass}, deps=${depsClass}; journal phases=[${phaseList}]). ` +
      `A foreign/unknown live hash is never auto-restored. Inspect the journal + ` +
      `temps + backups, reconcile the live files manually, then clear the writer lock.`,
    );
  }
  cleanup();
...
function safeUnlink(fsOps, file) {
  try {
    ...
    // Ignore only "file does not exist" (the temp/journal may never have been
    // written, or already cleaned up). Propagate permission/I/O errors
    // (EACCES/EIO/...) so a failed cleanup is never silently swallowed and the
    // command cannot report repin success while a journal/temp remains on disk.
...
export function repipeTransaction(sha, opts = {}) {
  ...
  try {
    ...
    // throw): the nonce-keyed release only deletes a lock that is still ours, so a
    // lock created by another writer in the gap is left untouched. A SIGKILL does
    // NOT run finally, so a crashed repin leaves a stale (dead-PID) lock + journal
    // for the operator to MANUALLY quarantine (mv) before --recover (O_EXCL-create
    // + reconcile) - that is the only path on which the lock outlives this
    // invocation. There is NO auto-reclaim anywhere in the protocol.
...
// realpathSync BOTH process.argv[1] and import.meta.url so a checker reached via a
// symlinked temp dir (macOS /var -> /private/var) still matches: Node canonicalizes
// the symlink in import.meta.url but NOT in process.argv[1], so a bare
// `path.resolve(argv[1]) === fileURLToPath(import.meta.url)` miscompares and check()
// never runs (exit 0, empty stdout) - the trap a cli-driver previously papered over.
// A non-file argv[1] (e.g. `node -e`) realpath-throws and is treated as not-main.
let isMain = false;
try {
  if (process.argv[1]) {
    isMain = fs.realpathSync(path.resolve(process.argv[1])) === fs.realpathSync(fileURLToPath(import.meta.url));
  }
} catch {
  isMain = false;
}
