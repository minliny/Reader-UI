...
// 用法：node frontend-demo-optimized/verify/verify-p0-chain-matrix.mjs
...
const CONTRACT_REPO = join(__dirname, "..", "..");
const WORKSPACE_ROOT = join(CONTRACT_REPO, "..");
const REPO_PATHS = {
  Contract: CONTRACT_REPO,
...
function verifyIOS(chain) {
  ...
  const generatedMotionRegistryTests = readText(
    join(testsDir, "ReaderAppTests", "ReaderGeneratedMotionRegistryTests.swift")
  ) || "";
  ...
  const hasGeneratedMotionRegistry =
    testRegex(motionAdapter, /MotionSpecRegistry\.spec\(for:/) &&
    testRegex(motionAdapter, /ReaderMotionResolver\.resolve\(request\)/) &&
    testRegex(generatedMotionRegistryTests, /Set\(ReaderUIContract\.MotionId\.allCases\)/) &&
    testRegex(generatedMotionRegistryTests, /XCTAssertEqual\(specs, generated\)/);
  const E = hasGeneratedMotionRegistry || testRegex(motionAdapter, motionPatterns[chain]);
...
function generateMarkdown(matrix) {
  ...
  lines.push("");
  lines.push("> 由 `frontend-demo-optimized/verify/verify-p0-chain-matrix.mjs` 自动生成");
  ...
  lines.push("- E. motion：ReaderMotionAdapter 使用完整生成式 Motion registry/resolver（并由 exact-set 测试锁定），或保留该链路的显式 motion 接线");
