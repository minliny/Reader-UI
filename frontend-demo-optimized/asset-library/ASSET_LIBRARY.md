...
| 补齐图标（Supplemented Icons） | 35 | 补齐 Shell、状态、Compose 语义追溯和缺少公共登记的图标 token。 |
...
- 阅读目录资产（Reader Directory Asset）：`reader-module-directory`
- 阅读朗读资产（Reader TTS Asset）：`reader-module-tts`
- 阅读界面资产（Reader Appearance Asset）：`reader-module-appearance`
- 阅读设置资产（Reader Settings Asset）：`reader-module-settings`
- 暂停（Pause）：`pause`
- 停止（Stop）：`stop`
...
| 阅读器专用图标（Reader Icon） | `fd-reader-actions .fd-medium-icon` | 24 px | 18-21 px | 作为当前资产登记，不定义入口组合或界面位置。 |
