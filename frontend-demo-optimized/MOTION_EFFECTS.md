...
- 本文件不定义阅读控制层的结构、模块内容、入口组合、展开关系或周边区域可见性；相关段落已移除，等待新的权威产品规格。
...
- 阅读器页面以正文稳定为最高优先级，正文排版不能被装饰动画打扰。
...
例子：打开底表未完成时触发弹窗，底表停止在当前状态并被收起或降级，弹窗按最高层级进入。

### `motion.interrupt.completeThenReplace`

使用场景：loading 中数据到达或异步检测结果返回。
...
| 形态 | 判断方式 | 布局目标 |
|---|---|---|
| 手机态 | 单屏窄宽度，或折叠屏合上 | 单列 app shell，底部主导航。 |
| 半展开态 | 宽度增加但高度受限，可能有 hinge/半开角度 | 保持单列优先。 |
...
3. `80ms -> 240ms`：Shell 容器从单列重排到展开态布局。主导航和顶栏使用 opacity/position 小幅过渡。
...
1. `0ms`：记录当前 active tab、route stack、ReaderContext、章节/进度锚点、active session、overlay 类型、focus owner 和可见胶囊类型。
2. `0ms -> 80ms`：取消非必要动画，包括翻页装饰、运行胶囊 pulse、TAB pressed、胶囊按钮 pressed 和底表/弹窗进入。
3. 阅读正文不淡出、不缩放；只冻结旧布局测量值，等待新尺寸下重新分页。
...
2. 主 TAB 栏和 segmented TAB 保持 active 状态；位置变化时只移动栏容器，不重建按钮。
3. 阅读正文按章节进度或字符锚点重新测量分页；正文可以即时替换到新排版，避免 opacity fade 造成闪烁或读者丢失阅读位置。
4. 如果沉浸阅读中有自动翻页/朗读胶囊，胶囊保持会话类型和播放态，重新锚定到新阅读 surface 的状态区域；不能跨 hinge 飞行。
5. overlay 以同语义容器映射：键盘跟随系统，底表/弹窗/换源窗口重新计算锚点；空间不足时降级为底表或全高面板，但保留状态和焦点语义。
...
3. 清理旋转临时状态，允许新的 TAB press、overlay enter 和翻页动画。
...
- 不重播 `app.launch.firstOpen` 或封面进入沉浸阅读。
...
- 正文和容器即时重排。
- overlay 和胶囊最多 `80ms` opacity 淡换。

验收：

- 横竖屏切换后 route、返回栈、active tab、ReaderContext 和 active session 不变。
...
使用场景：同一个 TAB 栏内的按钮状态变化，包括主导航 `书架/发现/RSS/设置` 以及设置/书源里的 segmented tab。
...
4. 内容区切换是另一个层级：主 TAB 内容可以 `80ms -> 120ms` 淡换，不让内容切换反向推动 TAB 栏。
...
1. open 状态下发生 scroll、keyboard、resize、fold、orientation 或宿主容器变化时，重新计算锚点和最大高度。
2. 空间不足时优先 drop-up；仍不足时限制最大高度并允许菜单内部滚动。
3. 如果当前锚点被键盘、其他覆盖层、hinge 或安全区遮挡，降级到底表/全高选择器，但选中值和焦点语义不丢。
4. 重定位不新增 route，不关闭当前页面，不让旧坐标残留。

禁用项：

- 不用整页遮罩伪装下拉，除非已经降级到底表/全高选择器。
- 不让菜单展开推挤设置行、书架列表、阅读正文或宿主容器高度。
...
使用场景：通用控件，包括 button、toggle、chip/filter、segment、slider、progress、stepper、input/search、toast/state、selection、list row、card、bookshelf view switch。
...
使用场景：用户启动自动翻页或朗读会话，然后回到沉浸阅读并显示运行胶囊。具体入口与周边产品结构不在本文定义。

效果时间线：

1. `0ms -> 80ms`：触发按钮进入 pressed/active 反馈。自动翻页可以突出倒计时/播放按钮，朗读可以突出播放按钮。
2. `120ms -> 200ms`：route 使用 replace 进入 `immersive-reading`，不新增返回栈层级。
3. `160ms -> 240ms`：运行胶囊在沉浸阅读状态区出现，透明度 `0 -> 1`，`translateY(6px) -> 0`，scale `0.96 -> 1`，时长 `160ms`。
4. 自动翻页胶囊显示倒计时圆点、`自动翻页` 文案和暂停/继续按钮；朗读胶囊显示朗读图标、`朗读` 文案和暂停/继续按钮。
...
- 不隐藏或重新创建胶囊。
- 不触发 route push 或 route replace。
- 不让点击按钮同时命中沉浸阅读正文热区。
...
使用场景：进入或退出换源 route。具体入口与周边产品结构不在本文定义。

打开效果：

1. 换源窗口从当前位置下方 `8px` 到原位，透明度 `0 -> 1`，时长 `200ms`。
2. 候选来源列表整体出现，不做过强 stagger；如需逐项进入，行间延迟不超过 `20ms`。
...
- 所有位移距离设为 `0`。
- route 和 overlay 使用即时显示/隐藏或不超过 `80ms` 的淡入淡出。
...
| TAB 按钮按下/选中/切换 | 主 TAB、segmented TAB | press、select、A -> B switch 可区分；栏尺寸、按钮坐标和点击热区稳定。 |
| 通用按钮/图标按钮 | 主按钮、危险按钮、行内按钮、toolbar 按钮 | press、activate、disabled blocked 一致；按钮尺寸和相邻控件稳定。 |
| Toggle/Switch/Checkbox | 设置开关、书源开关、批量勾选 | pressed、switch、revert 可区分；thumb/check/semantics 同步。 |
| Chip/Filter/Segment | 书架分组、Discover/RSS/Source 筛选、Settings segment | 即时选择、多选筛选、应用提交、segment switch 语义分开。 |
| Slider/Progress/Stepper | 数值调整、任务进度、恢复进度 | 拖动跟手无 easing；释放后 commit；进度更新不抖动。 |
...
| 下拉栏展开/收起/点击 | 设置页选项、发现排序、书源更多、书架更多、书籍焦点菜单 | trigger press、expand、collapse、option press、option select 统一；打开 A 后切 B 不残留；resize/orientation 可重定位。 |
...
| 自动翻页启动 | 开启自动翻页 session | route replace 回 `immersive-reading`，自动翻页胶囊进入，朗读状态被取消。 |
| 朗读启动 | 开始朗读 session | route replace 回 `immersive-reading`，朗读胶囊进入，自动翻页状态被取消。 |
| 运行胶囊更新/切换/退出 | 沉浸阅读中的自动翻页/朗读胶囊 | 倒计时和播放态只更新内部；互斥切换不排队；停止后释放点击热区。 |
| 胶囊暂停/继续按钮 | 自动翻页/朗读胶囊右侧控制按钮 | 按下只影响按钮；释放切换运行态；胶囊不重入场。 |
...
| 换源窗口 | 进入换源 route | 进入、退出与返回栈一致。 |
| 打断：关闭/返回 | 底表或弹窗进入中快速返回或关闭 | 旧动画不排队，最终状态唯一。 |
| 打断：loading 完成 + 返回 | 异步动作 loading 中立即返回 | 以返回为准，不把完成结果再盖回来。 |
| 折叠屏展开 | 手机态 reader/source-switch -> 展开态 | ReaderContext 保留，overlay 重新锚定。 |
| 折叠屏折叠 | 展开态 reader/source-switch -> 手机态 | 返回栈不新增记录。 |
| 横屏紧凑重排 | `compact-landscape` reader/settings | 当前 route 和状态不丢失。 |
...
6. 已完成第一版：首次打开应用有 cold-start 一次性 adapter；首启已有代表截图，下一步补默认页/深链页录屏和后台恢复证据。
...
13. 已完成第一版：`motion.interrupt.cancel/redirect/completeThenReplace` 已接入 root / screen host `data-motion-interrupt-*`、临时 pressed/dragging/dropdown 清理、route/Tab/viewport/loading/连续下拉 A->B 入口和 token 化短收尾；reader loading async result 已接入 `data-motion-async-*` request guard、cancel/discard 和完成态 CSS；Tab switch redirect 已有代表截图，下一步补 overlay 关闭、焦点恢复自动化和录屏证据。
...
15. 已完成第一版 `viewport.orientation.prepare/reshape/settle` adapter：root / screen host `data-motion-orientation-*`、route/session/overlay/focus 元数据、anchor settle CSS 和 reduced-motion 即时 settle 已接入；compact-landscape 已有代表截图，下一步补真实旋转录屏、正文字符锚点重分页和 overlay/focus 自动化证据。
