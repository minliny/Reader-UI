...
| 缺口 | 当前状态 | 需要补充 | 验收标准 |
|---|---|---|---|
| Motion ID 状态机表 | 只有触发点和效果描述 | 为每个 Motion ID 补 `from/to/interruption/fallback/final state` | 状态机表能解释所有打断和降级 |
...
- 不能声称通用按钮、chip/filter、toggle/switch、slider/stepper/progress、input/search、toast/state、selection、业务 row/card 已经完成实现层纳管；当前只有 selector 总表、基础 token/reduced-motion 和基础 pressed state 接入。
- 不能声称封面进入沉浸阅读已有录屏证据。
...
- 不能声称整屏旋转已有完整 `prepare/reshape/settle` 状态机、宽屏 dock clamp、运行胶囊/overlay 重锚定和录屏证据。
